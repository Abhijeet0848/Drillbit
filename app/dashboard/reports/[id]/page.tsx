...
export default async function ReportPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="animate-fade" style={{ display: 'flex', flexDirection: 'column', gap: '2rem', height: '100%' }}>
      ...
      <div className="report-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 350px', gap: '2.5rem', flex: 1, minHeight: 0 }}>
        {/* Document Viewer */}
        <div className="glass" style={{ 
          ...
          <div style={{ 
            ...
            <span style={{ fontWeight: 600, color: 'var(--primary)' }}>Verification Viewport</span>
            <div style={{ display: 'flex', gap: '1.5rem', fontSize: '0.8rem', fontWeight: 700 }}>
              <span style={{ color: 'var(--error)', textTransform: 'uppercase' }}>● Plagiarism Matches</span>
              <span style={{ color: 'var(--accent)', textTransform: 'uppercase' }}>● Neural AI Markers</span>
            </div>
          </div>
          <div className="printable-content" style={{ 
            ...
            {report.content ? report.content : "No readable content was extracted for this report."}
          </div>
        </div>

        {/* Sidebar Analysis */}
        <div className="no-print" style={{ display: 'flex', flexDirection: 'column', gap: '2.4rem' }}>
          {/* Similarity Chart */}
          <div className="glass" style={{ padding: '2.5rem 2rem', background: 'var(--bg-card)', textAlign: 'center', position: 'relative' }}>
             <h3 style={{ fontSize: '1rem', color: 'var(--text-muted)', marginBottom: '2rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Similarity Score</h3>
             <div style={{ 
                width: '180px', 
                height: '180px', 
                borderRadius: '50%', 
                ...
                position: 'relative',
                boxShadow: '0 0 20px hsla(0,0%,0%,0.05)'
             }}>
                <div style={{ 
                  width: '140px', 
                  height: '140px', 
                  ...
                  justifyContent: 'center'
                   ...
                   <span style={{ fontSize: '2.5rem', fontWeight: 800, color: 'var(--text-main)' }}>{simIndex}<small style={{ fontSize: '1rem' }}>%</small></span>
                   <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)', fontWeight: 700 }}>OVERLAP</span>
                </div>
             </div>
          </div>

          {/* AI Score */}
          <div className="glass" style={{ padding: '2.5rem 2rem', background: 'var(--bg-card)', textAlign: 'center' }}>
             <h3 style={{ fontSize: '1rem', color: 'var(--text-muted)', marginBottom: '2rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Neural Integrity</h3>
             <div style={{ 
                width: '180px', 
                height: '180px', 
                borderRadius: '50%', 
                ...
                <div style={{ 
                  ...
                  justifyContent: 'center'
                   ...
                   <span style={{ fontSize: '2.5rem', fontWeight: 800, color: 'var(--text-main)' }}>{aiScore}<small style={{ fontSize: '1rem' }}>%</small></span>
                   <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)', fontWeight: 700 }}>AI CONTENT</span>
                </div>
             </div>
          </div>

          {/* Top Matches */}
          <div className="glass" style={{ padding: '2rem', background: 'var(--bg-card)', flex: 1 }}>
            <h3 style={{ fontSize: '1rem', color: 'var(--primary)', marginBottom: '1.5rem', fontWeight: 700 }}>Global Source Matches</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {report.topSources && report.topSources.length > 0 ? (
                ...
                <div style={{ color: 'var(--text-muted)', fontSize: '0.9rem', padding: '2rem', textAlign: 'center' }}>Secure: No significant data leaks detected.</div>
              )}
            </div>
          </div>
        </div>
